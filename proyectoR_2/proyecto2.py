...
import json
import os
from cajaAzul_P2 import BlueBox
from configuracion_2 import ConfiguracionContainer
...
class UI(ft.Container):
    ...
    def obtener_datos_actualizados(self):
        """Simula la obtención de datos actualizados"""
        nuevos_datos = {
            'temperatura': self.datos_tiempo_real['temperatura'] + 0.5 if self.datos_tiempo_real['temperatura'] < 30 else 20,
            'humedad': self.datos_tiempo_real['humedad'] - 1 if self.datos_tiempo_real['humedad'] > 40 else 70,
            'presion': self.datos_tiempo_real['presion'] + 0.2 if self.datos_tiempo_real['presion'] < 100 else 90,
            'frecuencia': self.datos_tiempo_real['frecuencia'] + 0.1 if self.datos_tiempo_real['frecuencia'] < 65 else 60
        }
        
        # REDONDEAR LOS VALORES
        nuevos_datos['temperatura'] = round(nuevos_datos['temperatura'], 1)  # 1 decimal
        nuevos_datos['presion'] = round(nuevos_datos['presion'], 1)          # 1 decimal  
        nuevos_datos['frecuencia'] = round(nuevos_datos['frecuencia'], 1)    # 1 decimal
        nuevos_datos['humedad'] = round(nuevos_datos['humedad'])             # 0 decimales
